...
#             new_dict[word] = 0
    Doc = len(doc) ##總共幾篇文章
    Word = len(globals.new_dict) ##總共幾個字
    Query = 1 ##總共幾個query
    R = 5 ##relevant 數量
    NR = 3 ##最爛幾個Non-relevant
    EPOCH = 3 ##Rocchio跑幾次
#     globals.tf_idf_matrix = np.load('/home/nlp/Demo/api/numpy_IR/demo_tfidf.npy')
    idf_dict = globals.new_dict.copy()
    for q in range(Query):
        query_content = text
        query_tf = globals.new_dict.copy() ##紀錄tf分數
        update = {}
        query_content = jieba.cut(query_content)
        for word in query_content:
            if word in globals.new_dict:
                query_tf[word] += 1
                if word not in update:
                    idf_dict[word] += 1
                    update[word] = 0
        globals.tf_idf_matrix[q] = np.array(list(query_tf.values()))
    ans_list = []
    ans_index_list = []
    for i in range(Query): ##總共幾個query
        query_ans_list = []   ##排序輸出結果
        score_list = np.matmul(globals.tf_idf_matrix[Query:],globals.tf_idf_matrix[i]) 
